  ...
  <v-container fluid class="pa-0 fill-height">
    ...
    >
      ...
      >
        ...
        <template v-for="group in project.beatGroups" :key="`group-${group.id}-beats-container`">
          <BeatCard
        ...
        <BeatCard
          v-for="beat in project.beats.filter(
            b => !projectService.belongsToBeatGroup(project, b.id)
          )"
          :key="beat.id"
          :beat="beat"
    ...
    <!-- Properties Panel -->
    <PropertiesPanel :title="getPanelTitle()" orientation="vertical">
      <template #content>
...
function handleNewProject() {
  ...
  if (
    !confirm(t('toolbar.newProjectConfirm') || 'Create new project? Current work will be lost.')
  ) {
    return
  }
...
function handleCanvasTouchEnd(event: TouchEvent) {
  ...
  const touchEnded = !Array.from(event.touches).some(
    t => t.identifier === canvasTouchIdentifier.value
  )
...
function handleBeatDragStart(beatId: string) {
  ...
  const { project: updatedProject, startPosition } = dragAndDropService.handleBeatDragStart(
    project.value,
    beatId
  )
  project.value = updatedProject
...
function detectGroupHover(beatX: number, beatY: number) {
  ...
  for (const group of project.value.beatGroups) {
    ...
    for (const beatId of group.beatIds) {
      ...
      const absolutePos = positionCalculationService.getAbsoluteBeatPosition(
        groupBeat,
        project.value
      )
...
function handleBlockDragEnd(blockId: string) {
  ...
  if (targetBlockId && targetBlockId !== blockId) {
    ...
    if (draggingBlockLane && !isFirstInLane) {
      ...
      project.value = projectService.addBlockToLane(
        project.value,
        targetBlockLane.id,
        blockId,
        targetBlockId
...
function handleLaneDragMove(laneId: string, deltaX: number, deltaY: number) {
  ...
  blocksInLane.forEach(block => {
    ...
    project.value = projectService.updateBlock(project.value, block.id, {
      position: blockNewPosition
    })
    ...
    block.groupIds.forEach(groupId => {
      ...
      if (group) {
        ...
        project.value = projectService.updateBeatGroup(project.value, group.id, {
          position: groupNewPosition
        })
        ...
        group.beatIds.forEach(beatId => {
          ...
          if (beat) {
            ...
            project.value = projectService.updateBeat(project.value, beat.id, {
              position: beatNewPosition
            })
          }
        })
      }
